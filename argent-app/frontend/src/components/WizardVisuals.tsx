 ...
 * Adapted from Healthcare-EPIC-Snowflake-Demo for Argent Aerospace reliability analytics.
 *
 * Components:
 ...
 *
 * Zero-dep SVG and CSS. Aligned with the Argent Aerospace orange/navy palette.
 */
...
// ─────────────────────────────────────────────────────────────────────────
// Vendor / accent tokens — aligned with Argent Aerospace palette
// ─────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────
// WizardPipelineFlow — Hero pipeline
// ─────────────────────────────────────────────────────────────────────────
    ...
    <div className="wiz-flow relative">
      ...
      <style>{`
        ...
        .wiz-flow .wiz-stage:hover {
          ...
          border-color: var(--accent, ${C.orange});
          box-shadow: 0 14px 30px -18px rgba(26, 39, 68, 0.35);
        }
        .wiz-particle {
          display: block;
          width: 6px;
          ...
          background: linear-gradient(180deg, #fbd38d 0%, ${C.orange} 100%);
          box-shadow: 0 0 8px ${C.orange}cc;
          left: 0;
        ...
        <div className="md:col-span-1 flex md:flex-col items-center justify-center" aria-hidden>
          <div className="hidden md:flex relative w-full h-7 items-center">
            <div
              ...
              style={{ background: `linear-gradient(90deg, ${C.orange}00, ${C.orange}77, ${C.orange}00)` }}
            />
            <span
          ...
          <div className="md:hidden flex items-center justify-center w-full py-1">
            <svg width="14" height="20" viewBox="0 0 14 20" fill="none" aria-hidden>
              <path d="M7 1 L7 17 M1 11 L7 17 L13 11" stroke={`${C.orange}aa`} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      )}
...
const SPOKES: AgentSpoke[] = [
  { code: 'EX', name: 'Explorer',     tools: 'status · search',      blurb: 'Maps what exists',     color: C.orange, angle: -135 },
...
export function LineagePanel({
  ...
  return (
    <div className="argent-card flex flex-col" style={{ minHeight: 220 }}>
      ...
      <div className="flex-1 relative" style={{ minHeight: 180, padding: '14px 14px 12px' }}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full" role="img" aria-label="Live model lineage">
          ...
          {nodes.map((n) => {
            const fill = n.kind === 'silver' ? '#cfd6e0' : `${C.orange}`;
            const stroke = n.kind === 'silver' ? '#94a3b8' : (currentStep >= 6 || complete) ? C.bull : C.orange;
            return (
        ...
        <div className="absolute left-3 bottom-2 right-3 flex items-center justify-between font-mono" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>
          ...
          <span style={{ color: complete ? C.bull : C.orange }}>
...
export function BuildCompleteSummary({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
      {panels.map((p) => (
        <div key={p.title} className="argent-card p-4" style={{ borderTop: `3px solid ${p.accent}` }}>
...
// ─────────────────────────────────────────────────────────────────────────
// ModelRegistry — Argent Aerospace dbt model layout
// ─────────────────────────────────────────────────────────────────────────

type RegistryGroup = { layer: string; color: string; total: number; sample: string[]; };

const REGISTRY: RegistryGroup[] = [
  { layer: 'bronze', color: C.fivetran, total: 9,  sample: ['stg_maximo__work_orders', 'stg_sap__parts', 'stg_teamcenter__routes', 'stg_apriso__quality_events', 'stg_maximo__removals'] },
  { layer: 'silver', color: '#94a3b8',  total: 6,  sample: ['int_removal_event_spine', 'int_route_exposure', 'int_quality_findings', 'int_part_history'] },
  { layer: 'gold',   color: C.orange,  total: 9,  sample: ['fct_removal_event', 'fct_route_exposure', 'dim_part', 'fct_quality_event', 'fct_mro_cycle'] },
];

export function ModelRegistry({ newModelCode }: { newModelCode: string }) {
  const newLabel = newModelCode.replace(/^gold\./, '');
  return (
    <div className="argent-card p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {REGISTRY.map((g) => (
          <div key={g.layer} className="argent-card p-3" style={{ borderTop: `3px solid ${g.color}` }}>
      ...
      <p className="mt-3 font-mono" style={{ fontSize: 11, color: 'var(--ink-soft)' }}>
        ...
        Downstream consumers — airworthiness dashboards, CAMO reports, CRO briefings — pick it up on next read.
      </p>
      <style>{`
        .registry-new {
...
// ─────────────────────────────────────────────────────────────────────────
// LiveBuildThumbnail — Argent-adapted preview
// ─────────────────────────────────────────────────────────────────────────

const FRAMES = [
  { agent: 'EXPLORER',     color: C.orange, line1: 'dbt search "in-service removal rate route corridor"', line2: 'found 4 gold candidates' },
  { agent: 'WORKER',       color: C.rose,   line1: 'authoring fct_removal_by_part_route_corridor_60d.sql', line2: 'dbt_show ran on XS warehouse' },
