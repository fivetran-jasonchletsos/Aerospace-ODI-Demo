 ...
 * Route: /dbt-wizard
 *
 * Shows the CRO's question, a T-minus countdown to the Airworthiness Review,
...
export default function WizardScenarioPage() {
  ...
  useEffect(() => {
    let remaining = 12 * 3600; // 12-hour countdown to Airworthiness Review
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <span
            className="pill red inline-flex items-center gap-1.5"
            style={{ fontSize: 12, padding: '4px 10px', fontWeight: 700 }}
          >
            <span className="h-2 w-2 rounded-full bg-[var(--orange)] animate-pulse" />
            Gap · Active
          </span>
          <span className="mono text-xs text-[var(--ink-soft)] uppercase tracking-wider">{s.request_id}</span>
          <span className="mono text-xs text-[var(--ink-soft)] uppercase tracking-wider">Follows {s.prior_crisis_id}</span>
        </div>
        <h1 className="font-serif text-4xl sm:text-5xl font-semibold leading-[1.05] text-[var(--ink-strong)] tracking-tight">
          {s.timezone_label}.{' '}
          <span style={{ color: 'var(--orange)' }}>{s.requested_by}.</span>
        </h1>
        <p className="mt-3 max-w-3xl leading-relaxed text-lg text-[var(--ink-muted)]">
          No <span className="font-mono text-sm">gold.fct_removal_by_part_route_corridor_60d</span> exists.
          The {s.service_line} anomaly is unresolved.
          Airworthiness Review in 12 hours. Manual build ETA: {s.manual_time_days}.
        ...
        <div
          ...
          style={{ borderLeft: '4px solid var(--orange)', background: 'rgba(237,137,54,0.05)' }}
        >
          <div className="mono text-xs uppercase tracking-wider mb-2" style={{ color: 'var(--orange)' }}>The CRO's question</div>
      ...
      <div
        className="mb-8 rounded-lg border border-[var(--hairline)] p-4 flex items-center gap-4"
        style={{ borderLeft: '4px solid #dc2626', background: 'rgba(220,38,38,0.04)' }}
      >
        <div className="font-serif text-3xl font-semibold" style={{ color: '#dc2626' }}>$11M</div>
        <div>
          <div className="font-semibold text-[var(--ink-strong)]">AOG and fleet-grounding exposure</div>
          <div className="text-sm text-[var(--ink-muted)]">At risk while the removal-rate gap is unanswered and the Pacific fleet holds on ground.</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-10">
        <div className="lg:col-span-2 rounded-lg border border-[var(--hairline)] bg-white p-5 relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div>
              <div className="mono text-xs uppercase tracking-wider text-[var(--ink-soft)] mb-1">Upstream models available</div>
            ...
            {s.upstream_models.map(u => (
              ...
              >
                <div
                  ...
                  style={{ background: LAYER_COLOR[u.layer] ?? 'var(--orange)' }}
                />
                <div className="font-mono text-xs pl-1" style={{ color: LAYER_COLOR[u.layer] ?? 'var(--orange)' }}>
                  {u.layer}
                </div>
        ...
        <div className="rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm">
          <div className="mono text-xs uppercase tracking-wider text-[var(--ink-soft)] mb-3">State of the world</div>
          <dl className="space-y-3 text-sm">
            <Row k="Question requested by" v={s.requested_by} />
            <Row k="Requested at" v={<span className="font-mono">{s.timezone_label}</span>} />
            <Row k="Fleet corridor" v={s.service_line} />
            ...
            <Row k="Lookback window" v={<span className="font-mono">trailing 60 days</span>} />
      ...
      >
        <div className="mono text-xs uppercase tracking-wider mb-2" style={{ color: 'var(--orange)' }}>The path through six steps</div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-sm text-[var(--gunmetal-deep)] font-semibold px-6 py-4 whitespace-nowrap hover:opacity-95 transition-opacity"
...
const STEPS = [
  { title: 'Discovery',             who: 'Explorer',     tools: 'status, search',        color: 'var(--orange)' },
...
function KpiTile({
  ...
  return (
    ...
    >
      <div className="mono text-xs uppercase tracking-wider mb-2 text-[var(--ink-soft)]">{label}</div>
      <div
