 ...
 *
 * Ported from Healthcare-EPIC-Snowflake-Demo — Argent Aerospace-flavored.
 */
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          <span
            className="pill green inline-flex items-center gap-1.5"
            style={{ fontSize: 12, padding: '4px 10px', fontWeight: 700 }}
          >
            <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: '#145e36' }} />
            Build · Materialized
          </span>
          <span className="mono text-xs text-[var(--ink-soft)] uppercase tracking-wider">Lineage updated</span>
        </div>
      ...
      <section
        ...
        style={{ borderLeft: '5px solid var(--orange)', background: 'rgba(237,137,54,0.04)' }}
      >
        <div className="mono text-xs uppercase tracking-wider mb-2" style={{ color: 'var(--orange)' }}>Root cause identified</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <div className="mono text-xs uppercase tracking-wider mb-1 text-[var(--ink-soft)]">Affected cohort</div>
            <div className="font-mono text-sm font-semibold text-[var(--ink-strong)]">
              {o.root_cause.affected_cohort}
            </div>
          </div>
          <div>
            <div className="mono text-xs uppercase tracking-wider mb-1 text-[var(--ink-soft)]">Removal events</div>
            <div className="font-serif text-3xl font-semibold" style={{ color: 'var(--orange)' }}>
          ...
          <div>
            <div className="mono text-xs uppercase tracking-wider mb-1 text-[var(--ink-soft)]">Signature</div>
            <div className="font-mono text-sm font-semibold text-[var(--ink-strong)]">
              GKT-N41 gasket · Pacific corridor · cycles &gt; 400/month · post December
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-10">
        <LineagePanel
          ...
          subtitle="No gold table tracks removal rate by part, route corridor, over a 60-day window."
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <div className="rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm sm:col-span-2"
          ...
          <div className="mono text-xs uppercase tracking-wider mb-1 text-[var(--ink-soft)]">Materialized model</div>
        ...
        <div className="rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm"
          style={{ borderLeft: '4px solid #145e36' }}>
          <div className="mono text-xs uppercase tracking-wider mb-1 text-[var(--ink-soft)]">Tests</div>
          <div className="font-serif text-3xl font-semibold" style={{ color: '#145e36' }}>
      ...
      <section className="mb-10">
        ...
        <div className="rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm">
          <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {o.governance.map(g => (
              <div key={g.label}>
                <div className="mono text-xs uppercase tracking-wider mb-1 text-[var(--ink-soft)]">{g.label}</div>
      ...
      <section
        ...
        style={{ borderLeft: '5px solid #145e36', background: 'rgba(20,94,54,0.04)' }}
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center">
          <div className="sm:col-span-1">
            <div className="mono text-xs uppercase tracking-wider mb-2 text-[var(--ink-soft)]">dbt-wizard result</div>
            <div className="font-serif text-6xl sm:text-7xl font-semibold tracking-tight" style={{ color: '#145e36' }}>
              {o.hero.value}
            </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 rounded-lg border border-[var(--hairline)] bg-white p-5 items-center justify-between shadow-sm">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link
            to="/"
            className="inline-flex items-center gap-2 rounded-sm border border-[var(--hairline)] bg-white text-[var(--ink-strong)] font-semibold px-5 py-2.5 hover:bg-[var(--paper-deep)] transition-colors"
          >
            Back to home
          </Link>
          <Link
            to="/wizard-live"
            className="inline-flex items-center gap-2 rounded-sm text-[var(--gunmetal-deep)] font-semibold px-5 py-2.5 hover:opacity-95 transition-opacity"
...
function WizardColumn({ data, tone }: { data: Column; tone: 'crisis' | 'resolved' }) {
  const toneColor =
    tone === 'crisis' ? 'var(--orange)' : '#145e36';
  const pillClass =
    tone === 'crisis' ? 'pill amber' : 'pill green';
  return (
    <div
      className="rounded-lg border border-[var(--hairline)] bg-white p-6 shadow-sm"
      style={{ borderLeft: `5px solid ${toneColor}` }}
    >
      <div className={`${pillClass} mb-3 inline-flex`} style={{ fontSize: 11 }}>
        {data.label}
      </div>
      ...
      <div className="mono text-xs uppercase tracking-wider mb-2 text-[var(--ink-soft)]">Narrative</div>
...
function LineagePanel({
  ...
  for (const n of nodes) {
    const key = grouped[n.layer] ? n.layer : 'gold';
    grouped[key].push(n);
  }
  ...
  return (
    ...
    >
      <div className={`${pillClass} mb-2 inline-flex`} style={{ fontSize: 11 }}>
        {title}
      </div>
        ...
        {populated.map(layer => (
          <div key={layer}>
            <div
              ...
              style={{ color: NODE_COLOR[layer] ?? '#ed8936' }}
            >
              {layer}
            </div>
              ...
              {grouped[layer].map(n => {
                ...
                return (
                  <div
                    ...
                    style={{
                      borderLeft: `3px solid ${NODE_COLOR[layer] ?? '#ed8936'}`,
                      borderColor: 'var(--hairline)',
                      borderLeftColor: NODE_COLOR[layer] ?? '#ed8936',
                      background: isGap
                        ? 'rgba(220,38,38,0.06)'
                        : isNew
                        ? 'rgba(20,94,54,0.06)'
                  ...
                  >
                    <div className="font-mono text-[11px]" style={{ color: NODE_COLOR[layer] ?? '#ed8936' }}>
                      {layer}
                    </div>
                    ...
                    {isNew && (
                      <div className="font-mono text-[10px] mt-1" style={{ color: '#145e36' }}>
                        BUILT BY dbt-wizard
                      </div>
                    )}
                  </div>
      ...
      >
        ...
        <span className="text-[var(--hairline)]">&middot;</span>
